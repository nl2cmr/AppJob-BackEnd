...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    try {
        // Validation
        if (empty($data['idcertification'])) {
            http_response_code(400);
            echo json_encode([
                'success' => false,
                'message' => 'ID certification manquant'
            ]);
            exit;
        }
        ...
        // Suppression
        $stmt = $conn->prepare("DELETE FROM certification WHERE idcertification = ?");
        $stmt->bind_param("i", $data['idcertification']);
